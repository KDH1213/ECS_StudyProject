...
public class PlayerShooting : MonoBehaviour
{
	...
	void Start()
	{
		...
		EntityQuery query = new EntityQueryBuilder(Allocator.Temp).WithAll<SpawnDirectory>().Build(manager);

		if(query.HasSingleton<SpawnDirectory>())
		{
			bulletEntityPrefab = query.GetSingleton<SpawnDirectory>().bulletPrefab;
		}
	...
	void Update()
	{
        ...
        if (Mouse.current.leftButton.value > 0f && timer >= fireRate)
		{
	...
	void SpawnBulletECS(Vector3 rotation)
	{
		...
		manager.SetComponentData(bullet, localTransform);
	}
	...
	void SpawnBulletSpreadECS(Vector3 rotation)
	{
		...
		for (int x = min; x < max; x++)
		{
			...
			for (int y = min; y < max; y++)
			{
				...
				manager.SetComponentData(bullets[index], localTransform);
